...
//               initial={{ opacity: 0, y: 10 }}
//               animate={{ opacity: 1, y: 0 }}
//               className="clean-card rounded-2xl p-10 max-w-lg w-full mx-auto"
//             >
//               <div className="flex justify-end mb-4">
//                 <button
//                   onClick={() => setLang(lang === "en" ? "de" : "en")}
...
//                 </button>
//               </div>
//               <h1 className="text-2xl md:text-3xl font-style-broadway font-bold text-foreground mb-3 text-center mb-2">{t.title}</h1>
//               <p className="text-center text-muted-foreground mb-6">
//                 {t.subtitle}
//               </p>
...
import { submitFeedback } from "@/lib/firebase";
import { toast } from "sonner";
import dental from "@/public/assets/dental.png"
type Language = "en" | "de";
type FeedbackType = "like" | "dislike" | null;
...
const translations = {
  ...
  de: {
    ...
    improveTitle: "Es scheint, wir hätten es besser machen können.",
    improveSubtitle:
      "Es tut uns leid, dass Sie nicht ganz zufrieden waren.",
    improve:
      "Könnten Sie sich bitte einen Moment Zeit nehmen, um uns mitzuteilen, was wir verbessern könnten?",
    moreDetails:
      "Gibt es spezielles Feedback oder weitere Details, die Sie mit uns teilen möchten?",
  ...
  return (
    <div className="min-h-screen clean-bg flex items-center justify-center p-4">
      <motion.div
        ...
        animate={{ opacity: 1, y: 0 }}
        transition={{duration:0.5}}
        className="clean-card rounded-2xl p-8 md:p-10 max-w-lg w-full overflow-hidden"
      >
               {/* Top Header Row */}
...
{/* Center Logo */}
  <img
    ...
    alt="clinic"
    className="h-20 md:h-26 rounded-lg object-contain center"
  />
          <button
            onClick={() => setLang(lang === "en" ? "de" : "en")}
            className="flex items-center gap-2 text-sm text-primary"
        ...
        <AnimatePresence mode="wait">
          {feedbackType !== "dislike" ? (
            <motion.div
              ...
              animate={{ opacity: 1 }}
            >
              <h1 className="text-2xl md:text-3xl font-extralight text-center mb-3 font-serif text-[#4b2e2b]">
                {t.title}
              </h1>
            ...
            >
              ...
              <h3 className="text-2xl md:text-2xl font-extralight text-center mb-3 font-serif text-[#4b2e2b]">
                {t.improveTitle}
              </h3>
                ...
                {t.checkboxes.map((item) => {
                  ...
                  return (
                    <label
                      key={item}
                      onClick={() => toggleImprovement(item)}
                      className={`flex items-center gap-3 rounded-xl border p-4 text-sm cursor-pointer transition ${
                        selected
                          ? "bg-primary/10 border-primary"
                          : "hover:bg-muted border-border"
                      }`}
